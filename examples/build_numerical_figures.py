...
def mean_reversion(Phi, X, X_bar) -> None:
    ...
    axes[0].plot(h, 100 * EX[:, 0], "o-", color=BLUE, ms=4, lw=1.4)
    axes[0].axhline(100 * X_bar[0], color=GREY, ls="--", lw=0.9)
    axes[0].set_ylabel("growth (%)")
    axes[1].plot(h, 100 * EX[:, 1], "o-", color=RED, ms=4, lw=1.4)
    axes[1].axhline(100 * X_bar[1], color=GREY, ls="--", lw=0.9)
