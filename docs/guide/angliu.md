...
The six objects, in order, are the public files, the state $X_t$, the premium $\lambda_t$, the companion $\Phi$, the curve $\mu_t(n)$, and the perpetuity gap. December 2000 produces an upward curve because $\lambda_t$ is compressed and the state mean-reverts toward a higher long-run rate. Value’s curve sits above growth’s. The *level* of $\mu$ is not the same object as the *gap* versus a flat rate; a unit perpetuity isolates the denominator. The horizon split is the premium at the short end and persistent nominal states at the long end. That picture survived the post-2000 sample, and the gap is larger at the next cheap-premium date.
